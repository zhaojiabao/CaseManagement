	...
	<div class="every-page basicinfor">
		...
		<div style="background: #648fb4;">
			<div style="height: 65px;text-align: center;line-height: 65px;">
				...
				<Input readonly v-model="BaseInfo.name" style="width:300px;" placeholder="请输入案例名称"></Input>
			...
			<div style="height: 430px;border: 2px solid #fff; border-radius: 4px;padding: 10px 20px;margin: 0 20px;" class="clear-float">
				<div style="float: left;width: 530px;height: 100%;">
					<div class="clear-float" style="margin: 25px 0;">
						...
						<DatePicker readonly :editable='false' @on-open-change="startTimeChange" v-model='BaseInfo.start_time' type="datetime" placeholder="请输入起始时间" style="width: 158px"></DatePicker>
						<div class="input_info_text">结束时间：</div>
						<DatePicker readonly :options="endTimeOptions" :editable='false' @on-open-change="endTimeChange" v-model='BaseInfo.end_time' type="datetime" placeholder="请输入结束时间" style="width: 158px"></DatePicker>
					</div>
					<div class="clear-float" style="margin: 25px 0;">
						...
						<Cascader disabled style="width: 425px;display: inline-block;" placeholder='请选择地区' :data="maplist" v-model="address"></Cascader>
					</div>
					<div class="clear-float" style="margin: 25px 0;">
						<div class="input_info_text">场所简介：</div>
						<Input readonly v-model="BaseInfo.site_description" :rows="4" style="width: 425px;" type="textarea" placeholder="请输入场所简介" />
					</div>
					<div class="clear-float" style="margin: 25px 0;">
						<div class="input_info_text">事故后果：</div>
						<Input readonly v-model="BaseInfo.accident_description" :rows="4" style="width: 425px;" type="textarea" placeholder="主要从人员伤亡、环境破坏、财产损失等方面进行描述" />
					</div>
				</div>
				<div style="float: right;width: 530px;height: 100%;">
					<div class="clear-float" style="margin: 30px 0;">
						<div class="input_info_text">气象条件：</div>
						<Input readonly v-model="BaseInfo.weather_description" :rows="4" style="width: 425px;" type="textarea" placeholder="当日的天气、温度、风度、风向、风级等方面进行描述" />
					</div>
					<div class="clear-float" style="margin: 30px 0;">
						<div class="input_info_text">直接原因：</div>
						<Input readonly v-model="BaseInfo.direct_cause" :rows="4" style="width: 425px;" type="textarea" placeholder="请输入直接原因" />
					</div>
					<div class="clear-float" style="margin: 25px 0;">
						<div class="input_info_text">经验教训：</div>
						<Input readonly v-model="BaseInfo.experienced_lesson" :rows="4" style="width: 425px;" type="textarea" placeholder="请输入经验教训" />
					</div>
				</div>
			</div>
			...
			<div style="height: 80px;background: #FFF;margin-top: 70px;padding-top: 20px;position: relative;" class="clear-float">
...
<script>
	import { createBaseInfo, uploadImage,getCaseListDetail,AuditCase } from '../../../api/api'
		...
		methods: {
			...
			examineCase(id){
				AuditCase({
					...
					case_id:this.$route.params.user_look_case_id ,
					examine_id: id
				}).then(res => {
			...
			//去下一个详情
			goObjectInfo(){
				this.$router.push('/m/ObjectInfo/'+this.$route.params.user_look_case_id+"/"+this.mapId)
			},
			//返回上一个页面
			goBack(){
				this.$router.go(-1);
			...
			uploadInfo() {
				...
				createBaseInfo({
					...
					other_case_path: this.BaseInfo.other_case_path,
					img_path: this.BaseInfo.img_path
				}).then(res => {
					if(res.error_code == 2000) {
						this.$Message.success(res.error_message);
