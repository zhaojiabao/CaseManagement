<template>
	<div class="every-page edit-element">
		<div style="width: 800px;margin: 0 auto;margin-top:15px;background: #fff;padding: 20px;border-radius: 16px;">
			...
			<div class="yaosuzhi">
				...
				<div style="height: 230px;margin-top: 5px;" v-if="default_index==0">
					...
					<Button type="success" long icon='ios-add-circle-outline' @click="addLianxuAddElement()">添加要素</Button>
				</div>
				...
				<div style="height: 230px;margin-top: 5px;" v-else>
					...
					<Button type="success" long icon='ios-add-circle-outline' @click="addLisanAddElement()">添加要素</Button>
				</div>
				<div style="height: 60px;line-height: 60px;text-align: center;margin-top: 10px;clear: both;">
					<Button icon='ios-undo-outline' type="info" @click="goBack()">返回</Button>
			...
			<div style="border: 1px solid #EEEEEE;border-radius: 4px;margin: 15px 0;padding:0 20px;height: 450px;overflow: auto;">
				<div style="height: 100px;clear: both;" v-for="(item,index) in shareList">
					...
					<div style="width: 450px;height: 100px;display: inline-block;float: left;">
						<div style="height: 12px;width: 100%;margin-top: 45px;background: url(http://dphp.yysoul.com/img/toright.png);background-size: 100% 100%;position: relative;">
							<div v-for="(ele,i) in item.ele_content" :style="'height: 10px;width: 10px;border-radius: 5px;background:rgba(0,0,0,0.7);position: absolute;top: 1px;left:' +100/item.ele_content.length*i+'%'" :class="{changeBack:ele.selectvalue}" @click="clickShareDot(ele,index)">
								<div style="position: absolute; top: -25px;left: -95px;width: 200px;text-align: center;" v-if='ele.selectvalue'>
...
<script>
	import { getListElements, getSharesAllElements, addShareAllElements, createScene, getType, addMyElements, getSingleElements,editorElements } from '../../api/api'
	export default {
		data() {
			return {
				tabvalue: 'yiyou',
		...
		methods: {
			...
			getData(id) {
				getSingleElements({
					...
					id: this.$route.params.object_element_id,
					ele_object_type: this.$route.params.object_ele_object_type
				}).then(res => {
					if(res.error_code == 2000) {
						this.importantInfo.ele_name=res.data.ele_name;
						this.importantInfo.ele_dimension=res.data.ele_dimension;
						this.importantInfo.ele_min=res.data.ele_min;
						this.importantInfo.ele_max=res.data.ele_max;
						this.importantInfo.ele_desc=res.data.ele_desc;
						res.data.ele_content=JSON.parse(res.data.ele_content)
						if(Array.prototype.isPrototypeOf(res.data.ele_content[0].ele_protery_key)){
							this.default_index=0;
							this.lianxuList=res.data.ele_content;
						}else{
							this.default_index=1;
							this.lisanList=res.data.ele_content;
						}
					} else {
			...
			importantSave() {
				...
				editorElements({
					...
					object_id: this.$route.params.object_element_edit_id,
					category_id: this.$route.params.object_category_id,
