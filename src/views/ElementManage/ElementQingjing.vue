...
<script>
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				console.log(this.$route.params.element_scene_id)
				getElementByCategory({
					username: sessionStorage.getItem('username'),
					token: sessionStorage.getItem('token'),
					category_id: this.$route.params.element_scene_id,
					ele_object_type: 1
				}).then(res => {
			...
			getTypeInfo() {
				getTypeById({
					username: sessionStorage.getItem('username'),
					token: sessionStorage.getItem('token'),
					id: this.$route.params.element_scene_id
				}).then(res => {
			...
			//对象破坏要素
			addElementOne() {
				this.$router.push('/m/AddElement/' + 1 + '/' + this.$route.params.element_scene_id+'/'+0)
			},
			//灾害破坏要素
			addElementTwo() {
				this.$router.push('/m/AddElement/' + 2 + '/' + this.$route.params.element_scene_id+'/'+0)
			},
			//在属性列表中删除指定的元素
			delectElement(index) {
