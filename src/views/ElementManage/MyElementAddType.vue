<template>
	<div class="every-page qingjinkongjian" style="padding-top: 1px;">
		<div style="width: 800px;margin: 0 auto;margin-top:15px;background: #fff;padding: 20px;border-radius: 16px;" class="clear-float">
			...
			<div style="height: 30px;line-height: 30px;background: #00999b;border-radius: 15px;text-align: center;color: #fff;margin: 10px 0;">
				对象表达模型
			</div>
			...
			<div style="height: 30px;line-height: 30px;background: rgb(24,144,254);border-radius: 15px;text-align: center;color: #fff;margin: 10px 0;">
				情景空间
			</div>
			<div style="border: 1px solid #EEEEEE;border-radius: 4px;">
				<div style="height: 100px;clear: both;border-bottom: 1px solid #EEEEEE;">
					<div style="float: left;width: 200px;border-right: 1px solid #EEEEEE;height: 100%;text-align: center;">
						<p style="margin: 20px 0 10px;">对象破坏相关的要素</p>
					...
					<div style="height: 100%;float: left;width: 550px;overflow: auto;">
						<div style="width: 800px;height: auto;">
							<div v-for="(ele,index) in objectList" :key='index'>
								...
								<div class="float-left" style="width: 550px;padding-top: 45px;">
									<div style="position: relative;height: 8px;width: 100%;background: url(http://dphp.yysoul.com/img/toright.png);background-size: 100% 100%;">
										<div :style="'cursor: pointer;height: 6px;width: 6px;border-radius: 5px;background: rgba(0,0,0,0.7);position: absolute;top: 1px;box-shadow: 0 0 10px #FFFFFF;left:' +100/ele.ele_content.length*threeindex+'%;'" :key='threeindex' :class="{changeBack:three.selectvalue}" v-for="(three,threeindex) in ele.ele_content" @click="clickObjectDot(three,index)">
								...
								<div class="float-left" style="width: 100px;line-height: 100px;text-align: center;">
									<!--<Button type="info" size='small'>删除</Button>-->
								</div>
							</div>
						</div>
					</div>
				...
				<div style="height: 100px;clear: both;border-bottom: 1px solid #EEEEEE;">
					<div style="float: left;width: 200px;border-right: 1px solid #EEEEEE;height: 100%;text-align: center;">
						<p style="margin: 20px 0 10px;">灾害作用相关的要素</p>
					...
					<div style="height: 100%;float: left;width: 550px;overflow: auto;">
						<div style="width: 800px;height: auto;">
							<div v-for="(ele,index) in zaihaiList" :key='index'>
								...
								<div class="float-left" style="width: 550px;padding-top: 45px;">
									<div style="position: relative;height: 8px;width: 100%;background: url(http://dphp.yysoul.com/img/toright.png);background-size: 100% 100%;">
										<div :style="'cursor: pointer;height: 6px;width: 6px;border-radius: 5px;background: rgba(0,0,0,0.7);position: absolute;top: 1px;box-shadow: 0 0 10px #FFFFFF;left:' +100/ele.ele_content.length*threeindex+'%;'" :key='threeindex' v-for="(three,threeindex) in ele.ele_content" @click="clickZaiqingDot(three,index)">
											<div class="overflow-hidden" style="position: absolute; top: -45px;left: -95px;width: 200px;text-align: center;" :title="three.ele_protery_desc" v-if='three.selectvalue'>
												<span class="clickdotshow">{{three.ele_protery_desc}}</span>  <br />
												<Icon type="ios-pin" style='color:#1890FF;font-size: 20px;' />
											</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				console.log(this.$route.params.scene_id)
			...
			//对象破坏要素
			addElementOne() {
				this.$router.push('/m/MyAddElement/' + 1 + '/' + 0)
			},
			//灾害破坏要素
			addElementTwo() {
				this.$router.push('/m/MyAddElement/' + 2 + '/' + 0)
			},
			//在属性列表中删除指定的元素
			delectElement(index) {
