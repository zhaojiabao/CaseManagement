	...
	<div class="every-page basicinfor">
		...
		<div style="background: #648fb4;">
			...
			<div style="height: 80px;background: #FFF;margin-top: 70px;padding-top: 20px;text-align: right;" class="clear-float">
				<div style="display: inline-block;height: 50px;width: 100px;background: #13C2C2;background-size: 100% 100%;line-height: 50px;color: #FFFFFF;text-align: center;border-radius: 4px;cursor: pointer;" @click="goBack()">
					返回
				</div>
			</div>
		</div>
...
<script>
	import { createBaseInfo, uploadImage,getCaseListDetail } from '../../api/api'
		...
		methods: {
			...
			getCase(){
				getCaseListDetail({
					...
					case_id:this.$route.params.look_case_id
				}).then(res=>{
					console.log(res)
