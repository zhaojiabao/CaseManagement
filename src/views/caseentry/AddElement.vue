<template>
	...
	<div class="clear-float" style="height: 50px;">
		<Button class="float-left" type="info" @click="goBack()">返回</Button>
		...
		<div style="width: 800px;margin: 0 auto;margin-top:15px;background: #fff;padding: 20px;border-radius: 16px;">
			...
			<div class="yaosuzhi">
				...
				<div style="height: 230px;margin-top: 5px;" v-if="default_index==0">
					...
					<div class="clear-both">
						<div style="width: 80%;float:left;height: 160px;overflow: auto;">
							<div class="clear-float" v-for="(item,index) in lianxuList" :key='index'>
								<div style="width: 50%;float: left;height: 60px;line-height: 60px;">
									...
									<Input v-model="item.ele_protery_key[0]" style="width: 100px;" placeholder="请输入" />
									<div style="width: 80px;display: inline-block;text-align: center;">
										~
									</div>
									<Input v-model="item.ele_protery_key[1]" style="width: 100px;" placeholder="请输入" />
								</div>
								<div style="width: 50%;float: left;height: 60px;line-height: 60px;">
	...
	export default {
		...
		methods: {
			...
			importantSave() {
				...
				if(this.importantInfo.ele_name.length > 10) {
					this.$Message.error({
						content: '要素名称长度需小于10位！',
						duration: this.$store.state.duration
					});
					return;
				}
				...
				this.typeList.forEach((item, index) => {
					if(item.id == this.$route.params.category_id) {
				...
				createScene({
					...
					object_id: this.$route.params.add_element_object_id,
					...
					ele_object_type: this.$route.params.element_id,
					...
					if(res.error_code == 2000) {
						...
						addMyElements({
							...
							if(res.error_code == 2000) {
								...
								//								this.$router.push('/m/CreateQingjing/' + this.$route.params.category_id)
								this.$router.go(-1)
							} else {
								this.$Message.error({
									content: res.error_message,
