					...
					<div style="height: 100%;float: left;width: 550px;overflow: auto;">
						<div style="width: 750px;height: auto;">
							<div v-for="(ele,index) in objectList" :key='index'>
								...
								<div class="float-left" style="width: 450px;padding-top: 40px;">
								...
								<div class="float-left" style="width: 100px;line-height: 100px;text-align: center;">
									<Button type="info" size='small' @click="editClick(ele.object_id,ele.id,ele.category_id,1)">编辑</Button>
								</div>
							</div>
						</div>
					</div>
					...
					<div style="height: 100%;float: left;width: 550px;overflow: auto;">
						<div style="width:750px;height: auto;">
							<div v-for="(ele,index) in zaihaiList" :key='index'>
								...
								<div class="float-left" style="width: 450px;padding-top: 40px;">
									<div style="position: relative;height: 20px;width: 100%;background: url(http://dphp.yysoul.com/img/toright.png);background-size: 100% 100%;">
										<div :style="'cursor: pointer;height:10px;width: 10px;border-radius: 5px;background: rgba(0,0,0,0.7);position: absolute;top: 5px;box-shadow: 0 0 10px #FFFFFF;left:' +100/ele.ele_content.length*threeindex+'%;'" :key='threeindex' v-for="(three,threeindex) in ele.ele_content" @click="clickZaiqingDot(three,index)" :class="{changeBack:three.selectvalue}" >
											<div class="overflow-hidden" style="position: absolute; top: -45px;left: -95px;width: 200px;text-align: center;" :title="three.ele_protery_desc" v-if='three.selectvalue'>
												<span class="clickdotshow">{{three.ele_protery_desc}}</span>  <br />
												<Icon type="ios-pin" style='color:#1890FF;font-size: 20px;' />
											</div>
								...
								<div class="float-left" style="width: 100px;line-height: 100px;text-align: center;">
									...
									<Button type="info" size='small' @click="editClick(ele.object_id,ele.id,ele.category_id,2)">编辑</Button>
								</div>
							</div>
						</div>
					</div>
		...
		methods: {
			...
			getDataList() {
				...
				getElementByCategory({
					...
					category_id: this.$route.params.scene_id,
					ele_object_type: 1
				}).then(res => {
			...
			//对象破坏要素
			addElementOne() {
				this.$router.push('/m/AddElement/' + 1 + '/' + this.$route.params.scene_id)
			},
			//灾害破坏要素
			addElementTwo() {
				this.$router.push('/m/AddElement/' + 2 + '/' + this.$route.params.scene_id)
			},
			//在属性列表中删除指定的元素
			delectElement(index) {
				this.infoList.splice(index, 1);
			},
			editClick(object_id,element_id,type_id,id){
				this.$router.push('/m/ObjectEditElement/' + object_id + '/' + element_id + '/' + type_id + '/' + id)
			}
		}
	}
</script>
