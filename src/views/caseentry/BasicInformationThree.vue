	...
	<div class="every-page basicinfor" style="margin-top:0;">
		...
		<div style="background: #648fb4;height: 650px;position: relative;">
			...
			<div style="position: absolute;top: 150px;left: 0px;height: 180px;margin-left: 50px;line-height: 30px;">
				<span class="font-size-color">事故场所东西走向长度：{{changdu}} (m) </span> <br /> <span class="font-size-color"> 事故场所南北走向长度：{{kuandu}} (m) </span> <br /> <span class="font-size-color">单个网格边长：{{bianchang}} (m)</span>  <br />
			</div>
			<div id="this111" style='position: relative;width: 100%;height: 100%;'>
				...
				<!--<div style="position: absolute;top: 20%;left: 20%;cursor: pointer;">
					<div v-for="(i,index) in kuan" :style="'position:relative;width:'+400/objectChang+'px;height:'+400/objectChang+'px;display:inline-block'"">
						<span style="color: #FFF;margin-right: 5px;">{{index}}</span>
						<div v-for="j in chang" @click="selectTd((i-kuan)*-1+1+'-'+j)" :id="(j-kuan)*-1+1+'-'+i" class="default-td-background" :style="'position:relative;width:'+400/objectChang+'px;height:'+400/objectChang+'px;display:inline-block'">
							<span v-if="index==0" :style="'position: absolute;display: inline-block;top:'+400/objectChang+'px;width: '+400/objectChang+'px;left: 0;height: '+400/objectChang+'px;text-align: center;color: #FFF'">{{j}}</span>
						</div>
					</div>
				</div>-->
		...
		<div style="height: 80px;background: #FFF;margin-top: 20px;padding-top: 20px;text-align: right;" class="clear-float">
			<div v-if='editObject' style="display: inline-block;height: 50px;width: 200px;background: #648fb4;background-size: 100% 100%;line-height: 50px;color: #FFFFFF;text-align: center;border-radius: 4px;cursor: pointer;" @click="goScene()">
				下一步：前往录入情景与演化
			</div>
		</div>
		<!--点击创建对象-->
		...
		<Modal v-model="editObjectModel" width="800" :draggable='true'>
			...
			<div>
				<div class="clear-both" style="padding-bottom: 15px;border-bottom: 1px solid #EEEEEE;">
					<span class="objectinfo-span">类型 ：</span>
					<Select style="width:250px;float: left;" v-model="editObjectInfo.category_id">
						<Option v-if="typeList.length!=0" v-for="(item,index) in typeList" :key='index' :value="item.id">{{item.object_name}}</Option>
						<Option value='xinleixing'>+新类型+</Option>
					</Select>
					<span class="objectinfo-span">对象名称 ：</span>
					<Input v-model="editObjectInfo.object_name" placeholder="请输入对象名称" style="width: 250px;float: left;" />
				</div>
...
<script>
	import { createMap, getType, getTypeById, createObject, getAllByObject, getAllByScenario, getTheEntireMapInformation, deleteObject, editorObject } from '../../api/api'
	export default {
		data() {
		...
		mounted() {
			this.changdu = sessionStorage.getItem('shiguchangdu');
			this.kuandu = sessionStorage.getItem('shigukuandu');
